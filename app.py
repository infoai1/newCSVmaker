...
COMMON_FONTS = ["Arial", "Calibri", "Times New Roman", "Courier New", "Verdana", "Georgia", "Helvetica", "Tahoma", "Garamond", "Bookman"]
...
with st.sidebar:
    ...
    if st.session_state.uploaded_file_info:
        ...
        with st.expander("Chapter Heading Criteria", expanded=False):
            ...
            with col_ch_1:
                ch_check_font_props = st.checkbox("Enable Font Property Checks?", value=False, key="ch_check_font_props")
            ...
            with c_ch_1:
                ...
                ch_font_names = st.multiselect("Font Names (Chapter)", options=COMMON_FONTS, default=[], disabled=not ch_check_font_props, key="ch_font_names", help="Select one or more font names. Paragraph must contain at least one selected font. Best effort for DOCX.")
        ...
        with st.expander("Sub-Chapter Heading Criteria", expanded=False):
            ...
            with col_sch_1:
                sch_check_font_props = st.checkbox("Enable Font Property Checks?", value=False, key="sch_check_font_props")
            ...
            with c_sch_1:
                ...
                sch_font_names = st.multiselect("Font Names (Sub-Chapter)", options=COMMON_FONTS, default=[], disabled=not sch_check_font_props, key="sch_font_names", help="Select one or more font names. Best effort for DOCX.")
...
if process_button:
    ...
    # file_type = filename.split('.')[-1].lower() # Always docx now
    ...
    ch_heading_criteria = {
        ...
        'pattern_regex_str': st.session_state.ch_pattern_str,
        # DOCX specific: remove layout checks
        'check_layout': False,
    }
    ...
    with st.spinner(f"Processing '{filename}'... This may take a moment."):
        try:
            ...
            # pdf_skip options are not relevant for docx, so we pass 0 or defaults.
            # file_processor will only use _extract_docx
